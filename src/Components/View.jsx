...
const ViewRecipe = () => {
    ...
    return (
      <Container className="py-4">
        <div className="d-flex align-items-center mb-4">
          <Button variant="link" onClick={() => navigate(-1)} className="fs-4 fw-bold text-dark text-decoration-none p-0 me-2">
            <FaArrowLeft />
          </Button>
  ...
  return (
    <div className="view-recipe-container my-5 mx-auto px-3 px-md-0">
      ...
      <Row className="gx-5">
        <Col md={4}>
          <Image src={recipe.image}alt={recipe.title}fluidroundedclassName="mb-4 shadow-sm view-recipe-image"/>
          <div className="ingredients-list">
              <h5 className="mb-3 fw-semibold">Ingredients</h5>
              {(recipe?.ingredients ?.split(/[\n,]+/).map((item) => item.trim()).filter((item) => item.length > 0) || []).map((item, i) => (
              <p className="mb-1 ingredient-item" key={i}><strong>•</strong> {item}</p>
            ))}
          </div>
        </Col>
        <Col md={8}>
          <div className="method-section mb-4">
            ...
            <p>{recipe.method.length > 200? recipe.method.substring(0, 200) + "...": recipe.method}</p>
            <Button variant="link" className="ps-0"onClick={() => navigate(`/recipe-method/${id}`)} >
              View Full Method
            </Button>
          </div>
